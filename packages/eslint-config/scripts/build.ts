...
class Builder {
	...
	private async getRule(ruleName: string) {
		...
		const prefix =
			ruleName === 'simple-import-sort' ? '' : NAMESPACE_CONFIG[this.namespace].rulePrefix;
		const fullRuleName = prefix + ruleName;
		const comments = /\/\*\*.*\*\//gms.exec(fileContent);
		const fuleRuleKey =
			fullRuleName === 'simple-import-sort' ? 'simple-import-sort/imports' : fullRuleName;
		const rule: Rule = {
			name: fuleRuleKey,
			value: fileModule.rules[fuleRuleKey],
			description: '',
			reason: '',
			badExample: '',
			goodExample: '',
